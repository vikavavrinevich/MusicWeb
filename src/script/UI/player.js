...
function saveFavoriteTracks(favoriteTracks) {
    ...
    function toggleFavorite() {
        let favoriteTracks = getFavoriteTracks(); // Отримуємо улюблені треки
        const currentTrack = tracks[trackIndex]; // Поточний трек
        const isFavorite = favoriteTracks.some(favTrack => favTrack.title === currentTrack.title); // Перевіряємо, чи є поточний трек улюбленим
        
        if (isFavorite) {
            favoriteTracks = favoriteTracks.filter(favTrack => favTrack.title !== currentTrack.title); // Видаляємо трек зі списку улюблених
            likeTrackImg.src = "../assets/icons/heart.png"; // Змінюємо іконку на серце
        } else {
            const favoriteTrack = {
                src: currentTrack.src, // Додаємо URL треку
                ...
                id: currentTrack.id // Додаємо індекс треку
            };
